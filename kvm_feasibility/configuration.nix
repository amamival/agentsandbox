...
let
  ...
  Boot.Impermanence = {
    environment.persistence."/persistent" = {
      hideMounts = true;
      allowTrash = true;
      # In qemu:///session without subuid, /persistent is host user-owned.
      # Persisting system paths there makes /var and /etc fail root-owned checks.
      # "/etc/nixos"
      # "/var/lib/nixos"
      # "/var/log"
      # "/var/lib/libvirt"
      directories = lib.optionals (!sessionVm) [
        "/etc/nixos"
        "/var/lib/nixos"
        "/var/log"
  ...
  Shell = {
    environment.systemPackages = with pkgs; [
      ...
      lsof
      nixpkgs-fmt
      nodejs_24 # For Remote-SSH: "vscode@localhost -p 2223".
      perf
      ripgrep
      shellcheck
      sqlite
